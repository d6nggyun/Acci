...
public interface PrecedentRepository extends JpaRepository<Precedent, Long> {
    ...
    @Query("""
    SELECT new refresh.acci.domain.vectorDb.presentation.dto.res.PrecedentCasesResponse(
        p.caseName,
        p.summary,
        p.dateOfJudgment
    )
...
""")
    List<PrecedentCasesResponse> findAllPrecedentsByAnalysisId(@Param("analysisId") UUID analysisId);
